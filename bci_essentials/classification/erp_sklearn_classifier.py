"""**ERP sklearn Classifier**

This classifier is used to classify ERPs using an arbitrary sklearn-style
classifier (or pipeline) supplied at configuration time.

It mirrors `ErpRgClassifier`'s training loop (cross-validation, LICO,
oversampling/undersampling, optional channel selection) but lets the caller
swap in any estimator that implements `fit`, `predict`, and `predict_proba`.
...
def _default_clf():
    """Build the default XdawnCov(oas, oas, 3) -> TangentSpace -> LDA pipeline."""
...
class ErpSklearnClassifier(GenericClassifier):
    """ERP classifier wrapping any sklearn-style estimator
    ...
    def set_p300_clf_settings(
        ...
        Parameters
        ----------
        clf : sklearn-style estimator or Pipeline, *optional*
            Any object implementing `fit`, `predict`, and `predict_proba`.
            If `None`, defaults to
            `XdawnCovariances(estimator="oas", xdawn_estimator="oas",
            nfilter=3) -> TangentSpace -> LinearDiscriminantAnalysis(
        ...
        remove_flats : bool, *optional*
            Whether to prepend a `FlatChannelRemover` step.
            - Default is `True`.
        ...
        # Use the supplied classifier, or build the default pipeline.
        self.clf = clf if clf is not None else _default_clf()

        if remove_flats:
            rf = FlatChannelRemover()
            # If the supplied estimator isn't a Pipeline with a `steps` list,
            # wrap it so we can prepend the flat-channel remover.
            if not hasattr(self.clf, "steps"):
                self.clf = make_pipeline(self.clf)
    ...
    def fit(
        ...
        """
        logger.info("Fitting the model using sklearn pipeline")
        ...
        if self.channel_selection_setup:
            ...
            current_results = __erp_sklearn_kernel(X, self.y)
    ...
    def predict(self, X):
        ...
        prediction : Prediction
            Predict object. Contains the predicted labels and the probability.
            Because this classifier chooses the P300 object with the highest
            posterior probability, the probability returned is the posterior
            probability of the chosen object.
