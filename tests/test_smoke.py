...
from bci_essentials.classification.erp_rg_classifier_hyperparamgridsearch import (
...
class TestSmoke(unittest.TestCase):
    ...
    def test_p300_hyperparam_offline(self):
        # Get the P300 example data from ./examples/data
        xdf_path = os.path.join(os.path.dirname(__file__), "data", "p300_smoke.xdf")
        eeg_source = XdfEegSource(xdf_path)
        marker_source = XdfMarkerSource(xdf_path)
        ...
        # Select a classifier
        classifier = ErpRgClassifierHyperparamGridSearch()
        classifier.set_p300_clf_settings(
            n_splits=5,
            random_seed=42,
        )
        ...
        logger.info("P300 hyperparamer grid search test complete")
