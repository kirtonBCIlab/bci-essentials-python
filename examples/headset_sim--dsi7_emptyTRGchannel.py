...
while True:
    # Make separate eeg samples (random between 0 and 1) and trg samples (all zeros)
    eeg_sample = np.random.uniform(0.0, 1.0, n_eeg_channels)
    trg_sample = np.zeros(n_trg_channels)

    # Final sample is combination of this
    sample = np.append(eeg_sample, trg_sample)
